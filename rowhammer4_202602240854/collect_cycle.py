...
def run_single_cycle(workload, benchmark, cycle_num, is_hot, experiment_start_ns):
    ...
    stress_proc = None
    if is_hot:
        print(f"[*] Starting continuous heating to maintain 80°C during collection...")
        stress_proc = subprocess.Popen(
            ["stress-ng", "--cpu", "4", "--timeout", f"{CYCLE_DURATION + 10}s"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL
        )
    ...
    try:
        ...
        # Stop continuous heating if it was running
        if stress_proc:
            stress_proc.terminate()
            try:
                stress_proc.wait(timeout=5)
